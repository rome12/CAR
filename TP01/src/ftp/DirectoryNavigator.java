...
public class DirectoryNavigator {
	 ...
	 * @throws IOException
	 *             dans le cas ou le chemin n'est pas trouvé.
	 */
	...
	/**
	 * Permet de changer le répertoire courant pour le répertoire passé en
	 * paramètres.
	 * 
	 * @param path
	 *            le nouveau chemin, relatif ou absolu vers le dossier souhaité.
	 * @throws IOException
	 ...
	 * 
	 * @return le succès de la tache; Il renvoie faux dans le seul cas ou le
	 *         répertoire du serveur FTP n'est plus accessible.
	 */
	 ...
	 * @throws IOException
	 *             si un fichier/dossier n'est pas accessible.
	 * @throws NullPointerException
	 *             si des informations sont vide
	 */
	public String[] list_working_directory(String path) throws IOException {
		...
		for (File file : listOfFiles) {
			if (file.isFile()) {
				s.add("\053,r,i" + file.length() + ",\011" + file.getName()
						+ "\015\012");
			}
			if (file.isDirectory()) {
	 ...
	 * 
	 * @param path
	 *            le chemin à calculer
	 * @return une chaine de caractère contenant le chemin canonique du fichier
	 * @throws IOException
	 *             Si le chemin n'existe pas, ou n'est pas dans le répertoire
	...
	public String calculate_absolute_path(String path) throws IOException {
		...
		if (!path.startsWith(File.separator)) {
			file = new File(s + File.separator + this.working_directory
					+ File.separator + path);
		}
